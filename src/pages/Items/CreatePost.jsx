...
export default function CreatePost() {
    ...
    useEffect(() => {
        if (mode === 'found' && formData.visibility === 'match' && images.length > 0 && verificationsList.length === 0) {
            triggerSuggestQuestions(images[0]);
        }
    }, [formData.visibility]);
  ...
  const handleQuestionChange = (index, field, value) => {
    ...
    const handleSubmit = async (e) => {
        ...
        // Nếu đang sinh câu hỏi thì chờ
        if (mode === 'found' && formData.visibility === 'match' && isGeneratingQuestions) {
            setSubmitClicked(true);
            setNotification({ message: 'Hệ thống đang sinh câu hỏi, vui lòng chờ trong giây lát...', type: 'info' });
            return;
        }

        // Validation for step 2 in found mode (chỉ khi WHEN_MATCH)
        if (mode === 'found' && currentStep === 2 && formData.visibility === 'match') {
          ...
          {currentStep === 1 ? (
            <>
              ...
              {mode === 'found' && formData.visibility === 'match' && (
                <div className="pt-6 border-t border-outline-variant/30 mt-2 space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="font-label-bold text-label-bold text-on-surface-variant uppercase tracking-wider">Câu hỏi xác minh</h3>
                  ...
                  {verificationsList.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {verificationsList.map((q, index) => (
                        <div key={index} className="p-4 bg-surface-container rounded-xl border border-outline-variant/50 relative space-y-3 text-left">
                          <button type="button" onClick={() => handleRemoveQuestion(index)}
                            className="absolute top-3 right-3 text-red-500 hover:text-red-700 font-bold text-xs bg-red-50 hover:bg-red-100 px-2 py-1 rounded">
                            Xoá
                          </button>
                          <div className="space-y-1">
                            <label className="text-xs font-bold text-on-surface-variant uppercase tracking-wider block">Câu hỏi {index + 1} <span className="text-red-500">*</span></label>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div className="space-y-1">
                              <label className="text-xs font-bold text-on-surface-variant uppercase tracking-wider block">Đáp án đúng <span className="text-red-500">*</span></label>
                              <input type="text" value={q.correctAnswer}
                                onChange={(e) => handleQuestionChange(index, 'correctAnswer', e.target.value)}
                                placeholder="Ví dụ: Hình gấu bông màu hồng"
                                className="w-full px-3 py-2 rounded-lg border border-outline-variant bg-surface text-sm"
                                required />
                            </div>
                            <div className="space-y-1">
                              <label className="text-xs font-bold text-on-surface-variant uppercase tracking-wider block">Độ quan trọng</label>
                              <select value={q.importantPoint}
                                onChange={(e) => handleQuestionChange(index, 'importantPoint', e.target.value)}
                                className="w-full px-3 py-2 rounded-lg border border-outline-variant bg-surface text-sm cursor-pointer">
                      ...
                      <button type="button" onClick={handleAddQuestion}
                        className="w-full py-2.5 border-2 border-dashed border-primary/30 rounded-xl text-primary font-bold hover:bg-primary/5 transition-all flex items-center justify-center gap-2 text-sm">
                        + Thêm câu hỏi khác
                      </button>
                    </div>
                  )}
